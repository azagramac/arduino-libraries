...
int SE05XClass::SHA256(const uint8_t *buffer, size_t size, uint8_t *digest)
{
    beginSHA256();
    ...
    for(; bytes_read + 64 < size; bytes_read += 64, cursor += 64) {
        updateSHA256(cursor, 64);
    }
    updateSHA256(cursor, size - bytes_read);
    return endSHA256(digest, &outLen);
}
