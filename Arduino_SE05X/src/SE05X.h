...
public:
    ...
    /** SHA256
     *
     * Perform a oneshot SHA256 computation
     *
     * @param[in] buffer Input data buffer
     * @param[in] size size of the input data buffer
     * @param[out] digest Result of the SHA256 computation, it should be big enough (32 byte)
     *
     * @return 0 on Failure 1 on Success
     */
     ...
     * @param[in] hash Input SHA256 used to compute the signature
     * @param[in] hashLen SHA256 length
     * @param[in] sig Input buffer containing the signature
    ...
    /** AES_ECB_encrypt
     *
     * Encrypts something with AES ECB
     *
     ...
     * @param[in] sig Input buffer containing the signature R S values 64bytes
     ...
     * @param[in] message Input SHA256 used to compute the signature 32 bytes
     * @param[out] signature Output buffer containing the signature 64 bytes
     *
     * @return 0 on Failure 1 on Success
     */
    ...
    inline int writeConfiguration(const byte data[]=nullptr) { (void)data; return 1; }
